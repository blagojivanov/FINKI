...
class GenericCollection<T extends Comparable<T> & IHasTimestamp> {
    Map<String, Set<T>> collectionMap;
    ...
    void addGenericItem(String category, T element) {
        collectionMap.computeIfAbsent(category, x -> new TreeSet<>()).add(element);
    }

    public Collection<T> findAllBetween(LocalDateTime from, LocalDateTime to) {
        return collectionMap.values().stream().flatMap(Collection::stream).
                filter(x -> x.getTimestamp().isAfter(from) && x.getTimestamp().isBefore(to)).collect(Collectors.toCollection(() -> new TreeSet<T>(Comparator.reverseOrder())));
    }

    public Collection<T> itemsFromCategories(List<String> categories) {
        return collectionMap.entrySet().stream().filter(k -> categories.contains(k.getKey()))
                .map(Map.Entry::getValue).flatMap(Collection::stream).collect(Collectors.toCollection(() -> new TreeSet<T>(Comparator.reverseOrder())));
    }

    public Map<String, Set<T>> byMonthAndDay() {
        return collectionMap.values().stream().flatMap(Collection::stream).collect(Collectors.groupingBy(x ->
                        String.format("%02d-%02d", x.getTimestamp().getMonthValue(), x.getTimestamp().getDayOfMonth()),
                TreeMap::new,
    ...
    public Map<Integer, Long> countByYear() {
        return collectionMap.values().stream()
                ...
                .collect(Collectors.groupingBy(x -> x.getTimestamp().getYear(), TreeMap::new, Collectors.counting()));
    }
}
