...
class LogSystem<T extends ILog> {
    ...
    void printResults() {
        ...
        LogProcessor<T> firstLogProcessor = list -> list.stream().filter(log -> log.getType().equals("INFO")).collect(Collectors.toCollection(ArrayList::new));

        LogProcessor<T> secondLogProcessor = list -> list.stream().filter(log -> log.getMessage().length()<100).collect(Collectors.toCollection(ArrayList::new));


        LogProcessor<T> thirdLogProcessor = list -> list.stream().sorted().collect(Collectors.toCollection(ArrayList::new));
