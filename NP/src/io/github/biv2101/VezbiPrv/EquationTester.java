...
class EquationProcessor {
    public static <T, U> void process(List<T> inputs, List<Equation<T, U>> equations) {
        ...
        equations.stream().filter(e -> e.calculate().isPresent()).forEach(k -> System.out.println("Result: " + k.calculate().get()));
    }
}
...
public class EquationTester {
    public static void main(String[] args) {
        ...
        if (testCase == 1) { // Testing with Integer, Integer
            ...
            equations1.add(new Equation<>(() -> inputs.get(2), e -> inputs.get(2) + 1000));
            ...
            equations1.add(new Equation<>(() -> inputs.get(3), e -> Math.max(inputs.get(3), 100)));
            ...
            equations2.add(new Equation<>(() -> inputs.get(1), e -> inputs.get(1).calculateLine()));
            ...
            equations2.add(new Equation<>(() -> inputs.get(1), e -> inputs.stream().filter(x -> x.calculateLine() > inputs.get(0).calculateLine()).mapToDouble(Line::calculateLine).sum()));
